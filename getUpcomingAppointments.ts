...
export async function getUpcomingAppointments(authCookieKeyValue: string) {
    const response = await fetch("https://northwestbarberco.resurva.com/appointments", {
        headers: {
            "accept": "text/html",
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to get the appointments: ${JSON.stringify(response)}`);
    }
    ...
    const appointments = parseAppointments(root.querySelectorAll('#upcomingBookings ul li'));
...
function parseAppointments(htmlAppointments: HTMLElement[]): Appointment[] {
    return htmlAppointments.map(appointment => {
        ...
        let [dateString, timeString] = appointment.querySelectorAll('.title h4 strong').map(str => str.text.trim());
